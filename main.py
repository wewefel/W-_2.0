...
def bing_search(query, api_key, custom_config_id, num_results=8, excluded_site=''):
    search_query = f'intext:"{query}" -site:{excluded_site}' if excluded_site else f'intext:"{query}"'
...
def scrape_url(url):
    ...
    chrome_driver_path = r"C:\Users\wefel\chromedriver-win64\chromedriver-win64\chromedriver.exe" #This is an example. Change this to YOUR path to chromedriver.exe
...
def main():
    ...
    query = f"{company_name} company sustainability"
    excluded_site = input("Enter the site to exclude (e.g., nike.com): ")
    urls = bing_search(query, BING_API_KEY, CUSTOM_CONFIG_ID, excluded_site=excluded_site)
    print(f"Found {len(urls)} URLs")

    filtered_content = []

    with open("unfiltered.txt", "w", encoding="utf-8") as file:
    ...
    print("Scraping complete. Content saved to unfiltered.txt")
    ...
    print("Filtering complete. Content saved to filtered.txt")
